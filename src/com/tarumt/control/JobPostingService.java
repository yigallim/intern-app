...
public class JobPostingService implements Service {
    ...
    public ListInterface<JobPosting> filterBySalaryRange(ListInterface<JobPosting> jobPostings, int minSalary, int maxSalary) {
        ListInterface<JobPosting> filtered = new DoublyLinkedList<>();
        for (JobPosting job : jobPostings) {
            if ((job.getSalaryMax() >= minSalary) && (job.getSalaryMin() <= maxSalary)) {
                filtered.add(job);
            }
        }
        return filtered;
    ...
    public JobPosting.Type getJobTypeByChoice(int choice) {
        if (choice >= 1 && choice <= JobPosting.Type.values().length) {
            return JobPosting.Type.values()[choice - 1];
        } else {
            return JobPosting.Type.ACCOUNTING;  
        }
    }

    public ListInterface<JobPosting> filterByJobType(ListInterface<JobPosting> postings, JobPosting.Type type) {
        ListInterface<JobPosting> filtered = new DoublyLinkedList<>();
        for (JobPosting posting : postings) {
            if (posting.getType() == type) {
    ...
    public ListInterface<JobPosting> sortByDateCreated(ListInterface<JobPosting> jobPostings) {

        jobPostings.sort(new Comparator<JobPosting>() {
            @Override
            public int compare(JobPosting job1, JobPosting job2) {
                return job1.getCreatedAt().compareTo(job2.getCreatedAt());
            }
        });
        return jobPostings;
    }
    
    public ListInterface<JobPosting> sortByLastUpdated(ListInterface<JobPosting> jobPostings) {

        jobPostings.sort(new Comparator<JobPosting>() {
            @Override
            public int compare(JobPosting job1, JobPosting job2) {
                return job1.getUpdatedAt().compareTo(job2.getUpdatedAt());
            }
        });
        return jobPostings;
    }
